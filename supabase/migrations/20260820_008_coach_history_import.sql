-- Safe coach-only historical workout import.
-- Coaches retain read-only RLS on athlete-owned training tables; this narrowly scoped
-- security-definer function is the only write path for spreadsheet history import.
...
begin
  ...
  loop
    ...
    -- Duplicate identity is intentionally conservative: same athlete, workout name,
    -- and calendar date. Re-uploading the same workbook becomes a no-op.
    if exists (
    ...
    loop
      ...
      loop
        ...
        insert into public.sets(exercise_session_id,set_number,set_type,weight,reps,partial_reps,notes)
        values (
          v_exercise_session_id, v_set_number, 'working',
          greatest(0, coalesce((v_set->>'weight')::numeric,0)),
          greatest(0, coalesce((v_set->>'reps')::integer,0)),
          greatest(0, coalesce((v_set->>'partialReps')::integer,0)),
          case when coalesce((v_set->>'weight')::numeric,0) < 0
