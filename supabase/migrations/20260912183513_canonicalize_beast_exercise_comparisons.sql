-- Canonical-exercise identity integration for Beast.
-- Scoring math, skipped-exercise eligibility, lifecycle, and function contracts remain unchanged.
  ...
  if v_period.competition='eager_beaver' then
    ...
    with current_sessions as (
      ...
      select ce.workout_session_id,ce.athlete_user_id,ce.completed_at,
        ...
        )::integer current_eligible_set_count,
      ...
      select athlete_user_id,workout_session_id,min(completed_at) completed_at,
        ...
        count(*)::integer comparable_exercise_count
      from exercise_totals
      where current_eligible_set_count>0 and previous_total>0
...
-- Train Together uses the same current-work eligibility rule while retaining
-- its existing live/in-progress behavior and without writing official entries.
...
with allowed as (
 ...
 select mb.athlete_user_id,mb.athlete_name,mb.workout_session_id,mb.session_status,mb.completed_at,coalesce(cur_identity.canonical_exercise_id,cur.exercise_id) canonical_exercise_id,
 count(s.id) filter(where s.set_type::text not in ('warmup','timed') and coalesce(s.reps,0)>0 and coalesce(s.weight,0)>=0)::integer current_eligible_set_count,
 coalesce(sum(case when s.set_type::text not in ('warmup','timed') and coalesce(s.reps,0)>0 and coalesce(s.weight,0)>=0 then coalesce(s.weight,0)*s.reps else 0 end),0)::numeric current_total
 from members mb join public.exercise_sessions cur on cur.workout_session_id=mb.workout_session_id join public.exercises cur_identity on cur_identity.id=cur.exercise_id left join public.sets s on s.exercise_session_id=cur.id
 group by mb.athlete_user_id,mb.athlete_name,mb.workout_session_id,mb.session_status,mb.completed_at,coalesce(cur_identity.canonical_exercise_id,cur.exercise_id)
), comparable as (
 ...
 from current_ex c left join lateral (
   ...
   from public.workout_sessions pws join public.exercise_sessions pex on pex.workout_session_id=pws.id join public.exercises pex_identity on pex_identity.id=pex.exercise_id and coalesce(pex_identity.canonical_exercise_id,pex.exercise_id)=c.canonical_exercise_id join public.sets ps on ps.exercise_session_id=pex.id
 ...
 sum(greatest(current_total,0)) filter(where current_eligible_set_count>0 and previous_total>0) current_lift_total,
 sum(previous_total) filter(where current_eligible_set_count>0 and previous_total>0) previous_lift_total,
 count(*) filter(where current_eligible_set_count>0 and previous_total>0)::integer comparable_exercises
