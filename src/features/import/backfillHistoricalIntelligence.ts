...
type ExerciseSession={id:string;exercise_id:string;exercise_name_snapshot:string;workout_session_id:string;position:number;notes:string|null;sets:RawSet[];exercise:{canonical_exercise_id:string|null;name:string}|null};
...
export async function backfillHistoricalIntelligence(supabase:SupabaseClient,athleteUserId:string){
 ...
 const {data:exerciseData,error:exerciseError}=await supabase.from("exercise_sessions").select("id,exercise_id,exercise_name_snapshot,workout_session_id,position,notes,sets(id,weight,reps,partial_reps,set_type,set_number),exercise:exercises(canonical_exercise_id,name)").in("workout_session_id",ids);
 if(exerciseError)throw exerciseError;
 const exercises=(exerciseData??[]) as unknown as ExerciseSession[];
 ...
 for(const session of workoutSessions){
  ...
  const previousByIdentity=new Map(previous.map(ex=>[canonicalExerciseId(ex),ex]));
  const resultRows=current.map(ex=>{const prior=previousByIdentity.get(canonicalExerciseId(ex))??null;const result=scoreExercisePerformance({sets:normalize(ex.sets??[]),notes:combinedNotes(session.notes,ex.notes)},prior?{sets:normalize(prior.sets??[]),notes:combinedNotes(previousBundle?.session.notes,prior.notes)}:null);return{ex,prior,result};});
  for(const row of resultRows){
   ...
   const exerciseKey=canonicalExerciseId(row.ex);const history=historicalSetsByExercise.get(exerciseKey)??[];
  ...
  const first=current[0]??null;const priorFirst=first?previousByIdentity.get(canonicalExerciseId(first))??null:null;const firstSets=first?normalize(first.sets??[]).filter(s=>s.setType!=="warmup").slice(0,3):[];const priorFirstSets=priorFirst?normalize(priorFirst.sets??[]).filter(s=>s.setType!=="warmup").slice(0,3):[];
