...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      ...
      const [profileResult, latestResult, weeklySessionsResult, feedbackResult] = await Promise.all([
        ...
        supabase.from("coach_workout_feedback").select("workout_session_id, feedback, updated_at").eq("athlete_user_id", user.id).order("updated_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      ...
      if (feedbackResult.data) {
        const { data: feedbackWorkout } = await supabase.from("workout_sessions").select("workout_name_snapshot").eq("id", feedbackResult.data.workout_session_id).eq("athlete_user_id", user.id).maybeSingle();
        feedback = { ...feedbackResult.data, workout_name: feedbackWorkout?.workout_name_snapshot ?? null } as CoachFeedback;
      }
  ...
  return <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-8 px-6 py-10">
    ...
    {latestCoachFeedback && <section className="rounded-2xl border border-zinc-600 p-5"><p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-400">🤖 Incoming Coach Transmission</p><h2 className="mt-2 text-xl font-bold">Your coach left feedback{latestCoachFeedback.workout_name ? ` on ${latestCoachFeedback.workout_name}` : ""}.</h2><p className="mt-3 text-zinc-200">{latestCoachFeedback.feedback}</p><div className="mt-4 flex items-center justify-between gap-4"><p className="text-xs text-zinc-500">Updated {new Date(latestCoachFeedback.updated_at).toLocaleString()}</p><Link href={`/sessions/${latestCoachFeedback.workout_session_id}/report`} className="rounded-lg bg-white px-4 py-2 text-sm font-bold text-black">Open Transmission</Link></div></section>}
