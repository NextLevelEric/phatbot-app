...
function friendlyAuthMessage(mode: "signin" | "signup", error: { message?: string; code?: string } | null | undefined) {
  ...
  if (mode === "signin") {
    if (message.includes("email not confirmed") || code.includes("email_not_confirmed")) return "Your email is not confirmed yet. Open your PHATBOT confirmation email, confirm the account, then try signing in again.";
    if (message.includes("invalid login credentials") || message.includes("invalid credentials") || code.includes("invalid_credentials")) return "PHATBOT could not verify that email and password. Double-check the email, try your password again, or use Forgot password if needed.";
    if (message.includes("rate limit") || code.includes("rate_limit")) return "Too many sign-in attempts were made in a short period. Wait a few minutes, then try again.";
    return "PHATBOT could not sign you in. Please try again. If this keeps happening, use Forgot password or contact support.";
  }
  if (message.includes("already registered") || message.includes("user already registered") || code.includes("user_already_exists")) return "That email already has a PHATBOT account. Use Sign in instead, or use Forgot password if you do not remember the password.";
  if (message.includes("password") && message.includes("weak")) return "That password does not meet the account requirements. Choose a stronger password and try again.";
  if (message.includes("rate limit") || code.includes("rate_limit")) return "Too many signup attempts were made in a short period. Wait a few minutes, then try again.";
...
function AuthContent() {
  ...
  const [message, setMessage] = useState(invited ? "This athlete account was created for you. Open the PHATBOT invitation email to activate it and create your password." : "");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) { setMessage(friendlyAuthMessage(mode, result.error)); return; }
      ...
      await trackProductEvent("registration_completed", { dedupeKey: "lifetime" });
  ...
  async function resendConfirmation() {
    ...
    try {
      ...
      setMessage(error ? "PHATBOT could not resend the confirmation email right now. Please try again." : "Confirmation email resent. Check your inbox and spam folder.");
    } catch { setMessage("PHATBOT could not resend the confirmation email right now. Please try again."); }
    finally { setResending(false); }
  }

  return <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-12">
    <div className="mb-7 flex justify-center"><img src="/branding/PHAT%20BOT%20(3).png" alt="PHATBOT Fitness Robot" className="h-28 w-full max-w-sm object-contain sm:h-32" /></div>
    <p className="phat-accent text-sm font-semibold uppercase tracking-[0.25em]">PHATBOT Athlete</p>
    <h1 className="mt-2 text-4xl font-bold">{invited ? "Activate your PHATBOT account" : mode === "signup" ? "Create your account" : "Welcome back"}</h1>
    <p className="mt-3 text-zinc-300">{invited ? "Your PHATBOT workspace is ready for activation." : "Track the work. Beat the numbers. Keep improving."}</p>
    {invited && <div className="mt-6 rounded-xl border border-[rgba(255,0,50,.42)] bg-[rgba(255,0,50,.06)] p-4"><p className="font-semibold">Do not create another account.</p><p className="mt-2 text-sm text-zinc-300">Open the latest PHATBOT invitation email and click <strong>Activate PHATBOT Account</strong>. That link verifies your email and lets you create your password in one step.</p></div>}
    ...
    {!invited && mode === "signup" && awaitingConfirmation && <button onClick={resendConfirmation} disabled={resending} className="mt-4 rounded-lg border border-zinc-700 px-5 py-3 text-sm font-semibold disabled:opacity-60">{resending ? "Resending..." : "Resend Confirmation Email"}</button>}
    {!invited && mode === "signin" && <Link href="/auth/reset" className="mt-4 text-center text-sm text-zinc-300 underline">Forgot password?</Link>}
    {!invited && <button className="mt-6 text-sm text-zinc-300 underline" onClick={() => { setMode(mode === "signup" ? "signin" : "signup"); setMessage(""); setAwaitingConfirmation(false); }}>{mode === "signup" ? "Already have an account? Sign in" : "Need an athlete account? Create one"}</button>}
  </main>;
}
