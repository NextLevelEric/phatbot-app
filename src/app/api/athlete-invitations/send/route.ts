...
export async function POST(request: Request) {
  try {
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      to: athleteEmail,
      subject: `${coachName} invited you to PHATBOT`,
      html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:32px;color:#18181b"><p style="font-size:12px;letter-spacing:4px;font-weight:700">PHATBOT</p><h1 style="font-size:28px;margin:24px 0 12px">Your PHATBOT training space is ready.</h1><p style="font-size:16px;line-height:1.6;color:#52525b">${escapeHtml(coachName)} invited you to PHATBOT. Your coach can begin programming your workouts before you activate your account, so your training plan can be waiting when you arrive.</p><a href="${joinUrl}" style="display:inline-block;margin-top:20px;padding:14px 22px;background:#18181b;color:#fff;text-decoration:none;border-radius:8px;font-weight:700">Activate PHATBOT Account</a><p style="margin-top:28px;font-size:13px;color:#71717a">Use ${escapeHtml(athleteEmail)} when you sign in.</p></div>`,
    });
    ...
    return NextResponse.json({ ok: true, athleteUserId, status: existing ? "linked" : "invited" });
  } catch (error) {
