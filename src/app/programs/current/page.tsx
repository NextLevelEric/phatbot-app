...
export default function CurrentProgramPage() {
  ...
  if (!active) return <main className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center px-5 py-10"><section className="rounded-3xl border border-zinc-800 p-6 text-center"><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff0032]">No active program</p><h1 className="mt-2 text-2xl font-black">Train your way—or choose a rotation.</h1><p className="mt-3 text-sm text-zinc-400">Nothing is wrong. PHATBOT programs are optional.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><Link href="/programs" className="rounded-xl bg-[#ff0032] px-4 py-3 font-black">Browse Programs</Link><Link href="/workouts" className="rounded-xl border border-zinc-700 px-4 py-3 font-black">My Workouts</Link></div></section></main>;
  ...
  return <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6 sm:py-10">
    ...
    <section className="rounded-2xl border border-zinc-800 p-5"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-zinc-500">Assignment History</p><h2 className="mt-1 text-lg font-black">Programs stay in your history</h2></div><span className="rounded-full border border-zinc-700 px-3 py-1 text-xs font-black">{history.length}</span></div><div className="mt-4 flex flex-col gap-3">{history.map((item) => <div key={item.assignment_id} className="rounded-xl bg-zinc-950 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black">{item.program_family_name} <span className="text-xs text-zinc-500">v{item.version_number}</span></p><p className="mt-1 text-xs text-zinc-500">{formatAssignmentDate(item.started_at)} – {item.ended_at ? formatAssignmentDate(item.ended_at) : "Current"}</p><p className="mt-1 text-xs text-zinc-500">{assignmentSourceLabel(item)}</p></div><span className="text-[10px] font-black uppercase text-zinc-500">{item.assignment_status}</span></div></div>)}</div></section>
