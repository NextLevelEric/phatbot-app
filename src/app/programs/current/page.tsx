...
import { findDayOption, optionalDaysLabel, type ProgramDayOption } from "@/features/programs/optionalDays";
import {
...
export default function CurrentProgramPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const optionsResult = await supabase.rpc("get_program_day_options", { p_athlete_user_id: user.id });
      if (!mounted) return;
  ...
  return <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6 sm:py-10">
    ...
    <section><div><p className="text-xs font-black uppercase tracking-[.2em] text-zinc-500">Rotation</p><h2 className="mt-1 text-xl font-black">{rotation.map((day) => day.name).join(" → ")} → repeat</h2></div><div className="mt-4 flex flex-col gap-3">{rotation.map((day) => { const isNext = day.id === next?.dayId; const open = expanded === day.id; const option = findDayOption(dayOptions, active.assignment_id, day.id); return <article key={day.id} className={`rounded-2xl border ${isNext ? "border-[#ff0032] bg-[#ff0032]/5" : "border-zinc-800"}`}><button type="button" aria-expanded={open} onClick={() => setExpanded(open ? null : day.id)} className="flex w-full items-center justify-between gap-4 p-4 text-left"><div><p className="font-black">{day.name}{option?.is_optional && <span className="ml-2 text-xs font-bold text-zinc-400">Optional</span>}</p><p className="mt-1 text-xs text-zinc-500">{day.exercises.length} exercise{day.exercises.length === 1 ? "" : "s"}{isNext ? " · Up next" : ""}</p></div><span aria-hidden="true" className="text-zinc-500">{open ? "−" : "+"}</span></button>{open && <div className="border-t border-zinc-800 px-4 pb-4 pt-2">{day.exercises.length ? day.exercises.map((exercise) => <div key={`${day.id}-${exercise.position}`} className="border-b border-zinc-900 py-3 last:border-0"><div className="flex items-start justify-between gap-4"><p className="font-bold">{exercise.name}</p><p className="text-right text-sm text-zinc-300">{formatPrescriptionTargets(exercise.prescribed_set_targets)}</p></div>{exercise.notes && <p className="mt-1 text-xs leading-5 text-zinc-500">{exercise.notes}</p>}</div>) : <p className="py-3 text-sm text-zinc-400">No prescriptions are available for this workout.</p>}{option?.is_optional && <OptionalProgramDayActions option={option} disabled={!day.exercises.length} onSkipped={() => window.location.reload()} />}</div>}</article>; })}</div></section>
