...
export default function LiveWorkoutPage() {
  ...
  const [plateauByExercise, setPlateauByExercise] = useState<Record<string, PlateauSignal>>({});
  ...
  const loadSession = useCallback(async () => {
    ...
    const normalized = ((exerciseData ?? []) as ExerciseSession[]).map((item) => ({ ...item, prescribed_set_targets_snapshot: item.prescribed_set_targets_snapshot ?? [], sets: [...(item.sets ?? [])].sort((a,b) => a.set_number-b.set_number) }));
    ...
    if (sessionData.workout_id && normalized.length > 0) {
      ...
      if (previousSession) {
        ...
        for (const item of (previousExercises ?? []) as { exercise_id: string; sets: LoggedSet[] }[]) previousMap[item.exercise_id] = { completedAt: previousSession.completed_at ?? "", sets: [...(item.sets ?? [])].sort((a,b) => a.set_number-b.set_number) };
      }
    }
    const { data: plateauRows, error: plateauError } = await supabase.from("exercise_plateau_signals").select("exercise_id, consecutive_flat_sessions, change_percent").eq("athlete_user_id", user.id).eq("status", "active");
    if (plateauError) console.error("PHATBOT could not load plateau coaching", plateauError);
    const plateauMap = Object.fromEntries(((plateauRows ?? []) as PlateauSignal[]).map((row) => [row.exercise_id, row]));
    setSession(sessionData);
    setWorkoutNotes(sessionData.notes ?? "");
    setExercises(normalized);
    setPreviousByExercise(previousMap);
    setPlateauByExercise(plateauMap);
    setExerciseNotes(Object.fromEntries(normalized.map((item) => [item.id, item.notes ?? ""])));
    setLoading(false);
  }, [params.id]);
  ...
  async function completeWorkout() {
    ...
    try {
      setMessage(skipped.length ? `PHATBOT is completing the workout and excluding ${skipped.length} skipped exercise${skipped.length === 1 ? "" : "s"} from your score...` : "PHATBOT is locking in your workout and calculating the damage...");
      const supabase = createSupabaseBrowserClient();
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) { window.location.href = "/auth"; return; }
      const noteResults = await Promise.all([supabase.from("workout_sessions").update({ notes: workoutNotes.trim() || null }).eq("id", params.id).eq("status","in_progress"), ...exercises.map((exercise) => supabase.from("exercise_sessions").update({ notes: (exerciseNotes[exercise.id] ?? "").trim() || null }).eq("id", exercise.id))]);
      const noteError = noteResults.find((result) => result.error)?.error; if (noteError) { setMessage(noteError.message); return; }
      const { data, error } = await supabase.from("workout_sessions").update({ status: "completed", completed_at: new Date().toISOString() }).eq("id", params.id).eq("status", "in_progress").select("id").maybeSingle();
      if (error) { setMessage(error.message); return; }
      if (!data) { window.location.href = `/sessions/${params.id}/report`; return; }
      try { await syncAthletePlateauSignals(supabase, user.id); }
      catch (plateauError) { console.error("PHATBOT plateau sync failed after workout completion", plateauError); }
      window.location.href = `/sessions/${params.id}/report`;
