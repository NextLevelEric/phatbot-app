...
export default function WorkoutReportPage(){
  ...
  const loadReport=useCallback(async()=>{
    ...
    if(priorIds.length){
      const{data}=await supabase.from("exercise_sessions").select("workout_session_id, exercise_id, exercise_name_snapshot, position, notes, sets(weight, reps, partial_reps, set_type, set_number)").in("workout_session_id",priorIds);
      allPriorExercises=(data??[]) as (ExerciseSession&{workout_session_id:string})[];
    }
    ...
    setTopSetScores(firstSets.map((s,i)=>scoreExercisePerformance({sets:[s],notes:combinedNotes(currentSession.notes,first?.notes)},previousSets[i]?{sets:[previousSets[i]],notes:combinedNotes(priorById.get(priorFirst?.workout_session_id??"")?.notes,priorFirst?.notes)}:null)));
  ...
  return<main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6">
    ...
    <section className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-zinc-800 p-6 text-center"><p className="text-sm uppercase tracking-widest text-zinc-500">Progressive Overload Score</p><p className="mt-2 text-5xl font-black">{score===null?"BASELINE":`${score}%`}</p><p className="mt-3 text-sm text-zinc-400">{score===null?"No comparable performed exercises yet.":`${progressed} progressed · ${neutral} neutral · ${regressed} regressed`}</p></div><div className="rounded-2xl border border-zinc-800 p-6 text-center"><p className="text-sm uppercase tracking-widest text-zinc-500">Strength vs Last Workout</p><p className="mt-2 text-5xl font-black">{strengthPercent===null?"N/A":`${strengthPercent>=0?"+":""}${strengthPercent.toFixed(1)}%`}</p><p className="mt-3 text-sm text-zinc-400">{strengthChange?.comparableExerciseCount?`${strengthChange.comparableExerciseCount} comparable exercise${strengthChange.comparableExerciseCount===1?"":"s"} · ${weightUnit} × full reps`:"Complete this workout again to establish a comparison."}</p></div></section>
    <section className="rounded-xl border border-zinc-800 p-4 text-sm text-zinc-400"><strong className="text-zinc-200">Two useful comparisons:</strong> PO scoring follows the last time you actually performed each exercise. Strength change still compares this workout with the previous completion of the same workout using common exercises only.</section>
