...
function combinedNotes(workoutNotes: string | null | undefined, exerciseNotes: string | null | undefined) { return [workoutNotes, exerciseNotes].filter(Boolean).join(" ").trim() || null; }
function scoreSingleSet(current: PerformanceSet, previous: PerformanceSet | null, currentNotes?: string | null, previousNotes?: string | null) { return scoreExercisePerformance({ sets: [current], notes: currentNotes }, previous ? { sets: [previous], notes: previousNotes } : null); }
...
export default function WorkoutReportPage() {
  ...
  const [strengthChange, setStrengthChange] = useState<StrengthChangeResult | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    const { data: previousSession } = await supabase.from("workout_sessions").select("id, notes").eq("athlete_user_id", user.id).eq("workout_id", currentSession.workout_id).eq("status", "completed").lt("completed_at", currentSession.completed_at).order("completed_at", { ascending: false }).limit(1).maybeSingle();
    ...
    const currentTopNotes = combinedNotes(currentSession.notes, firstExercise?.notes);
    const previousTopNotes = combinedNotes(previousSession?.notes, previousFirst?.notes);
    setTopSetScores(firstSets.map((set, index) => scoreSingleSet(set, previousSets[index] ?? null, currentTopNotes, previousTopNotes)));
  ...
  const allPRs = rows.flatMap((row) => row.prs.map((pr) => ({ ...pr, exercise: row.name })));
  const strengthPercent = strengthChange?.percentageChange ?? null;
  const qualityProtected = rows.some((row) => ["lower_weight_improved_quality", "fewer_reps_improved_quality"].includes(row.result.explanationCode)) || topSetScores.some((row) => ["lower_weight_improved_quality", "fewer_reps_improved_quality"].includes(row.explanationCode));

  return <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6">
    <header><p className="text-sm font-semibold uppercase tracking-[0.25em] text-zinc-400">PHATBOT Workout Report</p><h1 className="mt-2 text-3xl font-bold">{session.workout_name_snapshot}</h1><p className="mt-2 text-zinc-400">Did you improve today?</p></header>
    ...
    {coachFeedback.length > 0 && <section className="rounded-2xl border border-zinc-600 p-6"><p className="text-sm font-bold uppercase tracking-[0.2em] text-zinc-400">🤖 Incoming Coach Transmission</p><h2 className="mt-2 text-2xl font-bold">Your coach left feedback.</h2><p className="mt-2 text-sm text-zinc-400">Beep boop. Human coaching signal received.</p><div className="mt-4 flex flex-col gap-3">{coachFeedback.map((item) => <div key={item.id} className="rounded-xl bg-zinc-900 p-5"><p className="whitespace-pre-wrap text-base leading-7 text-zinc-100">{item.feedback}</p><p className="mt-3 text-xs text-zinc-500">Updated {new Date(item.updated_at).toLocaleString()}</p></div>)}</div></section>}
