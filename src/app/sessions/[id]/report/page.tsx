...
export default function WorkoutReportPage() {
  ...
  const comparable = rows.filter((row) => row.result.result !== "baseline");
  const score = comparable.length ? Math.round((comparable.reduce((sum, row) => sum + row.result.score, 0) / comparable.length) * 100) : null;
