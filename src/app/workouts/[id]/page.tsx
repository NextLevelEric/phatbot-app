...
export default function WorkoutDetailPage() {
  ...
  async function removeExercise(id: string) {
    ...
    if (error) setMessage(error.message); else {
      const remaining = workoutExercises.filter((item) => item.id !== id);
      await Promise.all(remaining.map((item, index) => supabase.from("workout_exercises").update({ position: index + 1 }).eq("id", item.id)));
      setMessage("Exercise removed. Sequence recalibrated."); await loadPage();
    }
    setWorking(false);
  }
  ...
  async function moveExercise(index: number, direction: -1 | 1) {
    ...
    const supabase = createSupabaseBrowserClient();
    const updates = await Promise.all(reordered.map((item, position) => supabase.from("workout_exercises").update({ position: position + 1 }).eq("id", item.id)));
    const error = updates.find((result) => result.error)?.error;
    if (error) setMessage(error.message); else { setMessage("Beep boop. Workout order updated."); await loadPage(); }
    setWorking(false);
  }

  async function saveRepTargets(item: WorkoutExercise) {
    const draft = repDrafts[item.id] ?? { min: "", max: "" };
    const min = Number(draft.min); const max = Number(draft.max);
    ...
    const { error } = await supabase.from("workout_exercises").update({ target_rep_min: min, target_rep_max: max }).eq("id", item.id);
    if (error) setMessage(error.message); else { setMessage(`Beep boop. ${item.exercise.name} target locked at ${min}-${max} reps.`); await loadPage(); }
    setWorking(false);
  }
