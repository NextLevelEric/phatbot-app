...
export default function WorkoutDetailPage() {
  ...
  const isCoachAssigned = Boolean(coachId);
  ...
  return <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-8 px-6 py-10">
    ...
    {isCoachAssigned ? <section className="rounded-2xl border border-zinc-800 p-6">
      <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-500">Today's Training</p><h2 className="mt-2 text-xl font-semibold">Exercise Preview</h2><p className="mt-1 text-sm text-zinc-400">Your coach set this training sequence. Start the workout when you are ready.</p></div>
      {workoutExercises.length === 0 ? <p className="mt-6 rounded-xl border border-dashed border-zinc-700 p-6 text-center text-zinc-400">No exercises are assigned to this workout yet.</p> : <ol className="mt-6 flex flex-col gap-3">{workoutExercises.map((item,index) => <li key={item.id} className="flex items-center gap-4 rounded-xl border border-zinc-800 p-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-zinc-800 text-sm font-semibold">{index+1}</span><div className="min-w-0 flex-1"><p className="font-semibold">{item.exercise.name}</p><p className="mt-1 text-sm text-zinc-400">{item.exercise.muscle_group ?? "Exercise"}{item.exercise.equipment ? ` · ${item.exercise.equipment}` : ""}</p>{fallbackTarget(item).length > 0 && <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-zinc-500">Target: {fallbackTarget(item).join(", ")} reps</p>}</div></li>)}</ol>}
    </section> : <section className="rounded-2xl border border-zinc-800 p-6"><div className="flex items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">Exercises</h2><p className="mt-1 text-sm text-zinc-400">Review or edit the exercise order and targets below.</p></div><button onClick={() => setShowAdd((v) => !v)} className="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-black">{showAdd ? "Done" : "+ Add Exercise"}</button></div>
