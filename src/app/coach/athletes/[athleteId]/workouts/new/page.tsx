...
export default function CoachNewWorkoutPage(){
 ...
 async function submit(e:FormEvent){e.preventDefault();setSaving(true);setMessage("");const trimmed=name.trim();if(!trimmed){setMessage("Give the workout a name.");setSaving(false);return;}const supabase=createSupabaseBrowserClient();const{data,error}=await supabase.from("workouts").insert({athlete_user_id:athleteId,name:trimmed,description:description.trim()||null,is_active:true}).select("id").single();if(error){setMessage(error.message);setSaving(false);return;}window.location.href=`/coach/athletes/${athleteId}/workouts/${data.id}`;}
