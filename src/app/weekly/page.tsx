...
type WeeklyWorkout = { id: string; name: string; completedAt: string; score: number | null; strengthChange: number | null };
...
export default function WeeklyPage() {
  ...
  useEffect(() => { const supabase = createSupabaseBrowserClient(); async function load() {
    ...
    for (const session of (sessions ?? []) as WorkoutSession[]) {
      ...
      if (!previousSession) { results.push({ id: session.id, name: session.workout_name_snapshot, completedAt: session.completed_at, score: null, strengthChange: null }); continue; }
      ...
      results.push({ id:session.id, name:session.workout_name_snapshot, completedAt:session.completed_at, score:score === null ? null : Math.round(score*100), strengthChange: strength.percentageChange });
    }
  ...
  const strengthScored = workouts.filter((workout) => workout.strengthChange !== null); const weeklyStrength = strengthScored.length ? strengthScored.reduce((sum,workout) => sum + (workout.strengthChange ?? 0),0)/strengthScored.length : null;
  return <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-6 py-10"><header><p className="text-sm font-semibold uppercase tracking-[0.25em] text-zinc-400">PHATBOT Weekly</p><h1 className="mt-2 text-3xl font-bold">Your week in training</h1><p className="mt-2 text-zinc-400">Workout scores and strength changes roll up into a simple weekly view.</p></header>{message && <p className="rounded-xl border border-zinc-800 p-4 text-sm">{message}</p>}<section className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-zinc-800 p-6 text-center"><p className="text-sm uppercase tracking-widest text-zinc-500">Weekly Performance Score</p><p className="mt-2 text-5xl font-black">{weeklyAverage === null ? "BASELINE" : `${weeklyAverage}%`}</p><p className="mt-3 text-sm text-zinc-400">{scored.length} comparable workout{scored.length === 1 ? "" : "s"} · {workouts.length} completed</p></div><div className="rounded-2xl border border-zinc-800 p-6 text-center"><p className="text-sm uppercase tracking-widest text-zinc-500">Avg Strength vs Prior</p><p className="mt-2 text-5xl font-black">{weeklyStrength === null ? "N/A" : `${weeklyStrength >= 0 ? "+" : ""}${weeklyStrength.toFixed(1)}%`}</p><p className="mt-3 text-sm text-zinc-400">Average change across {strengthScored.length} comparable workout{strengthScored.length === 1 ? "" : "s"}</p></div></section><section className="flex flex-col gap-3">{workouts.length === 0 ? <div className="rounded-xl border border-zinc-800 p-5 text-zinc-400">No completed workouts yet this week.</div> : workouts.map((workout) => <Link key={workout.id} href={`/sessions/${workout.id}/report`} className="rounded-xl border border-zinc-800 p-5"><div className="flex items-center justify-between gap-4"><div><p className="font-semibold">{workout.name}</p><p className="mt-1 text-xs text-zinc-500">{new Date(workout.completedAt).toLocaleDateString()}</p><p className="mt-2 text-sm text-zinc-400">Strength: {workout.strengthChange === null ? "N/A" : `${workout.strengthChange >= 0 ? "+" : ""}${workout.strengthChange.toFixed(1)}%`}</p></div><p className="text-2xl font-bold">{workout.score === null ? "Baseline" : `${workout.score}%`}</p></div></Link>)}</section><section className="rounded-xl border border-zinc-800 p-4 text-sm text-zinc-400">Weekly strength is the average of each comparable workout's load × full-reps percentage change versus the prior completion of that same workout. Warmups are excluded.</section><div className="grid gap-3 sm:grid-cols-2"><Link href="/progress" className="rounded-lg border border-zinc-700 px-5 py-3 text-center font-semibold">Progress Trend</Link><Link href="/" className="rounded-lg bg-white px-5 py-3 text-center font-semibold text-black">Back to Dashboard</Link></div></main>;
}
