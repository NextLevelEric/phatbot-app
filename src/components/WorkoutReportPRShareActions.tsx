...
import { canonicalExerciseId, canonicalExerciseName, type ExerciseIdentityRelation } from "@/features/exercises/identity";

type RawSet={weight:number;reps:number;partial_reps:number;set_type:string};
type ExerciseRow={exercise_id:string;exercise_name_snapshot:string;sets:RawSet[];exercise?:ExerciseIdentityRelation|ExerciseIdentityRelation[]|null};
...
export default function WorkoutReportPRShareActions({sessionId}:{sessionId:string}){
 ...
 useEffect(()=>{let dead=false;async function load(){const s=createSupabaseBrowserClient();const{data:{user}}=await s.auth.getUser();if(!user)return;const{data:session}=await s.from("workout_sessions").select("completed_at, workout_name_snapshot").eq("id",sessionId).eq("athlete_user_id",user.id).maybeSingle();if(!session)return;const{data:current}=await s.from("exercise_sessions").select("exercise_id, exercise_name_snapshot, sets(weight,reps,partial_reps,set_type), exercise:exercises(canonical_exercise_id,name)").eq("workout_session_id",sessionId);const{data:priorSessions}=await s.from("workout_sessions").select("id").eq("athlete_user_id",user.id).eq("status","completed").lt("completed_at",session.completed_at);const ids=(priorSessions??[]).map(r=>r.id);let prior:ExerciseRow[] = [];if(ids.length){const{data}=await s.from("exercise_sessions").select("exercise_id, exercise_name_snapshot, sets(weight,reps,partial_reps,set_type), exercise:exercises(canonical_exercise_id,name)").in("workout_session_id",ids);prior=(data??[]) as unknown as ExerciseRow[];}const out:SharePR[]=[];for(const e of (current??[]) as unknown as ExerciseRow[]){const identity=canonicalExerciseId(e),history=prior.filter(p=>canonicalExerciseId(p)===identity).flatMap(p=>p.sets??[]);for(const pr of detectPersonalRecords(prs(e.sets??[]),prs(history))){out.push({key:`${identity}-${pr.type}-${pr.weight}-${pr.reps}`,exercise:canonicalExerciseName(e),weight:pr.weight,reps:pr.reps,detail:pr.type==="heaviest_weight"?"New heaviest weight":"New rep PR"});}}if(!dead)setItems(out);}void load();return()=>{dead=true}},[sessionId]);
