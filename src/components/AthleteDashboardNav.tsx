...
const secondaryLinks = [
  ...
  { href: "/weekly", label: "Weekly Report", description: "Review this week's training signal." },
];
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Link
          href={activeWorkoutId ? `/sessions/${activeWorkoutId}` : "/workouts"}
          className="rounded-2xl bg-white p-5 text-black transition hover:bg-zinc-200"
        >
        ...
        <Link
          href="/progress"
          className="rounded-2xl border border-zinc-700 p-5 transition hover:border-[#ff0032]"
        >
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {secondaryLinks.map((item) => (
          <Link key={item.href} href={item.href} className="rounded-xl border border-zinc-800 p-4 transition hover:border-zinc-600">
            <p className="font-bold">{item.label} <span className="text-[#ff0032]">→</span></p>
      ...
      {latestWorkoutId && (
        <Link href={`/sessions/${latestWorkoutId}/report`} className="text-sm font-semibold text-zinc-300 underline decoration-zinc-600 underline-offset-4">
          Open latest workout report →
        </Link>
      )}
    </section>
  );
