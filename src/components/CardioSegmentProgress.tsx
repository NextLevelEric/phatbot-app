...
function parentLabel(activity:Activity|undefined, segment:Segment){if(!activity)return"Recorded cardio effort";const mi=miles(activity.distance_meters);const larger=activity.distance_meters!=null&&activity.distance_meters>segment.distance_meters*1.08;return larger&&mi?`${segment.segment_label} inside a ${mi.toFixed(1)} mi ${activity.activity_name??"workout"}`:`${activity.activity_name??"Cardio"} effort`;}
...
export default function CardioSegmentProgress(){
  ...
  const groups=useMemo(()=>{const map=new Map<string,Segment[]>();for(const segment of segments){const list=map.get(segment.segment_key)??[];list.push(segment);map.set(segment.segment_key,list);}return [...map.entries()].map(([key,list])=>{const chronological=[...list].sort((a,b)=>new Date(activities[b.cardio_activity_id]?.started_at??0).getTime()-new Date(activities[a.cardio_activity_id]?.started_at??0).getTime());const latest=chronological[0],previous=chronological[1]??null,best=[...list].sort((a,b)=>Number(a.duration_seconds)-Number(b.duration_seconds))[0];return{key,label:latest.segment_label,latest,previous,best,count:list.length};}).sort((a,b)=>a.latest.distance_meters-b.latest.distance_meters);},[segments,activities]);
  if(loading||groups.length===0)return null;
  return <section className="mx-auto mb-8 w-full max-w-2xl px-4 sm:px-6"><div className="rounded-3xl border border-[#ff0032]/30 bg-[#ff0032]/5 p-5"><p className="text-[10px] font-black uppercase tracking-[.2em] text-[#ff0032]">CARDIO PROGRESSION V2</p><h2 className="mt-2 text-2xl font-black">Comparable efforts inside every run.</h2><p className="mt-2 text-sm leading-6 text-zinc-400">PHATBOT now treats standard distances as benchmarks even when they happen inside a longer activity.</p><div className="mt-5 grid gap-3">{groups.map(group=>{const latestActivity=activities[group.latest.cardio_activity_id],bestActivity=activities[group.best.cardio_activity_id],delta=group.previous?Number(group.previous.duration_seconds)-Number(group.latest.duration_seconds):null;return <div key={group.key} className="rounded-2xl border border-zinc-800 bg-black/10 p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.16em] text-zinc-500">{group.label}</p><p className="mt-1 text-3xl font-black">{clock(Number(group.latest.duration_seconds))}</p><p className="mt-1 text-xs text-zinc-500">{parentLabel(latestActivity,group.latest)}</p></div><div className="text-right"><p className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">BEST</p><p className="mt-1 text-lg font-black text-[#ff0032]">{clock(Number(group.best.duration_seconds))}</p><p className="mt-1 text-[10px] text-zinc-600">{bestActivity?new Date(bestActivity.started_at).toLocaleDateString():""}</p></div></div>{delta!==null?<p className={`mt-3 text-sm font-bold ${delta>0?"text-[#ff0032]":"text-zinc-400"}`}>{delta>1?`${clock(delta)} faster than the previous ${group.label}.`:delta < -1?`${clock(Math.abs(delta))} slower than the previous ${group.label}.`:"Essentially matched the previous effort."}</p>:<p className="mt-3 text-sm text-zinc-500">Baseline established. The next comparable segment creates a trend.</p>}<p className="mt-2 text-[10px] uppercase tracking-wide text-zinc-600">{group.count} comparable effort{group.count===1?"":"s"} recorded</p></div>})}</div></div></section>;
}
