...
export default function LiveWorkoutMenu(){
 ...
 useEffect(()=>{if(!sessionId)return;let active=true;const s=createSupabaseBrowserClient();void(async()=>{
  const[{data:session},{data:{user}}]=await Promise.all([s.from("workout_sessions").select("started_at").eq("id",sessionId).maybeSingle(),s.auth.getUser()]);
  if(active&&session?.started_at)setStartedAt(session.started_at);
 ...
 useEffect(()=>{if(!startedAt)return;const tick=()=>setElapsed(Math.max(0,Math.floor((Date.now()-new Date(startedAt).getTime())/1000)));tick();const id=window.setInterval(tick,1000);return()=>clearInterval(id);},[startedAt]);
 ...
 return <div className="fixed bottom-24 right-4 z-50 sm:bottom-6 sm:right-6">
  {open&&<div className="mb-2 w-[min(21rem,calc(100vw-2rem))] rounded-2xl border border-zinc-800 bg-black/95 p-4 shadow-2xl backdrop-blur">
   ...
   <div className="mt-3 rounded-xl border border-zinc-800 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Train Together</p>{room?<><p className="mt-1 font-black">{room.room_name||"Live Workout Room"}</p><p className="mt-1 text-xs font-bold text-[#ff0032]">CONNECTED · IN ROOM</p></>:<p className="mt-1 text-sm text-zinc-400">Start or join a live room race.</p>}</div>{room?<Link href={`/train-together/${room.join_code}`} className="rounded-lg border border-[#ff0032]/50 px-3 py-2 text-xs font-black text-[#ff0032]">VIEW ROOM</Link>:<Link href={`/sessions/${sessionId}/train-together`} className="rounded-lg border border-zinc-700 px-3 py-2 text-xs font-black">OPEN</Link>}</div></div>
  </div>}
