...
export default function CoachAthleteProgramSection({ athleteId }: { athleteId: string }) {
  ...
  return <section className="rounded-2xl border border-zinc-800 p-5">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#ff0032]">Program</p><h2 className="mt-1 text-xl font-black">{active ? `${active.program_family_name} v${active.version_number}` : "No program assigned"}</h2>{active && <><p className="mt-1 text-sm text-zinc-400">{assignmentSourceLabel(active)} · Assigned {formatAssignmentDate(active.started_at)}</p><p className={`mt-2 text-sm font-bold ${review.isDue ? "text-amber-300" : "text-zinc-500"}`}>{review.label}</p></>}</div><button type="button" onClick={() => setShowForm((value) => !value)} className="rounded-xl bg-[#ff0032] px-4 py-3 text-sm font-black text-white">{active ? "Change Program" : "Assign Program"}</button></div>
    ...
    {history.length > 0 && <div className="mt-4"><button type="button" onClick={() => setShowHistory((value) => !value)} className="text-sm font-black text-zinc-300">{showHistory ? "Hide History" : "View History"} →</button>{showHistory && <div className="mt-3 flex flex-col gap-2">{history.map((item) => <div key={item.assignment_id} className="rounded-xl bg-zinc-950 p-3"><p className="font-bold">{item.program_family_name} <span className="text-xs text-zinc-500">v{item.version_number}</span></p><p className="mt-1 text-xs text-zinc-500">{formatAssignmentDate(item.started_at)} – {item.ended_at ? formatAssignmentDate(item.ended_at) : "Current"} · {assignmentSourceLabel(item)}</p></div>)}</div>}</div>}
  </section>;
}
