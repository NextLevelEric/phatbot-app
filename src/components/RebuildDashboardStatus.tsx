...
type HealthSnapshot = { startDate?: string; endDate?: string; restingHeartRate?: number | null; hrvMs?: number | null; activeEnergyKcal?: number; steps?: number; dailyMetrics?: DailyHealthMetric[]; workouts?: HealthWorkout[]; };
...
async function persistHealthSnapshot(snapshot: HealthSnapshot) {
  ...
  // HealthKit does not disclose read-denial status. Avoid treating an empty
  // authorization result as real zero activity and overwriting prior history.
  const dailyMetrics = (snapshot.dailyMetrics ?? []).filter((day) => day.date && ((day.steps ?? 0) > 0 || (day.activeEnergyKcal ?? 0) > 0));
...
export function RebuildDashboardStatus() {
  ...
  return <>
    {healthAvailable && <section className="rounded-2xl border border-zinc-700 bg-zinc-950 p-5"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#ff0032]">Apple Health</p><div className="mt-2 flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold">{healthConnected ? "Apple Health Connected" : "Connect Apple Health"}</h2><p className="mt-2 text-sm leading-6 text-zinc-300">{healthConnected ? "PHATBOT can sync your Apple Watch and Health activity history. Selected daily activity and workout details are saved to your PHATBOT account. You do not need to reconnect when you leave this screen." : "PHATBOT requests read-only access to steps, active energy, workouts, workout distance and heart rate, resting heart rate, and HRV. Selected daily activity and workout details are saved to your PHATBOT account for activity history and training trends. Apple Health is optional, and PHATBOT never writes to Apple Health."}</p></div>{healthConnected && <span className="shrink-0 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[10px] font-black tracking-wider text-emerald-300">CONNECTED</span>}</div>
